...
StripLPD8806::StripLPD8806(uint16_t n) {
  // use hardware SPI!
  dataPin = clockPin = 0;
  hardwareSPI = true;
...
StripLPD8806::StripLPD8806(uint16_t n, uint8_t dpin, uint8_t cpin) {
  dataPin = dpin;
  clockPin = cpin;
...
uint16_t StripLPD8806::numPixels(void) {
  return numLEDs;
}

uint32_t StripLPD8806::Color(byte r, byte g, byte b)
{
...
inline void StripLPD8806::write8(uint8_t d) {
...
// Basic, push SPI data out
void StripLPD8806::writezeros(uint16_t n) {
  // this is the 'least efficient' way (28ms per 32-LED write)
  /*
...
void StripLPD8806::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
  uint32_t data;
...
void StripLPD8806::setPixelColor(uint16_t n, uint32_t c) {
  if (n >= numLEDs) return; // '>=' because arrays are 0-indexed
